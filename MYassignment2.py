...
def correlation_heatmap(
        ...
        cmap='coolwarm'):
    '''
    Fetches data from the World Bank API, creates a correlation matrix using NumPy,
    and creates a correlation heatmap using Seaborn.
    '''
    # retrieve the data from the World Bank API using requests
    url = 'http://api.worldbank.org/v2/country/' + country_code + '/indicator/'
    data = {}
    for key, value in indicators.items():
        query = url + key + '?format=json&date=' + start_date + ':' + end_date
        response = requests.get(query)
        if response.status_code == 200:
            json_data = response.json()[1]
            data[value] = [x['value'] for x in json_data]
    df = pd.DataFrame(data, index=range(
        len(data[list(indicators.values())[0]])))
    ...
    coolwarm_palette = sns.color_palette(cmap, as_cmap=True)
    sns.heatmap(
        corr_matrix,
        annot=True,
        cmap=coolwarm_palette,
        xticklabels=list(
            indicators.values()),
        yticklabels=list(
            indicators.values()))
...
if __name__ == '__main__':
    ...
    co2_df = create_bar_graph(
        countries,
        co2_data,
        'CO2 emissions (2010-2020)',
        'CO2 emissions (kt)')

    indicators = {'SP.URB.TOTL.IN.ZS': 'Urban Population',
                  'NY.GDP.MKTP.KD.ZG': 'GDP Growth',
                  'EN.ATM.CO2E.KT': 'CO2 emissions'}

    # create a correlation heatmap for China
    correlation_heatmap('CHN', '2000', '2022', indicators, cmap='coolwarm')
